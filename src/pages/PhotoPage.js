      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '20px' }}>
        {filteredPhotos.length > 0 ? (
          filteredPhotos.map(photo => (
            <div key={photo.id} style={{ textAlign: 'center', border: '1px solid #ddd', padding: '10px', borderRadius: '5px' }}>
              <img src={photo.thumbnailUrl} alt={photo.title} style={{ width: '100px', height: '100px', objectFit: 'cover' }} />
              <p>{photo.title}</p>
            </div>
          ))
        ) : (
